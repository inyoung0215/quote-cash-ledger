...
이 레포가 조건부 UPDATE 를 고른 근거를 **재봤다.**
숨고 블로그의 안심번호 할당 문제를 재현해 여섯 가지 방법을 같은 조건에서 측정했다.

| 전략 | 처리량/s | p99 | 데드락 |
|---|---|---|---|
| `for_update` | 246 | 442ms | 0 |
| **`skip_locked`** | **1488** | **31ms** | 0 |
| `conditional_update` | 183 | 1727ms | 0 |
| `conditional_update_spread` | 680 | 399ms | **28** |
| `conditional_update_spread_tx` | 728 | 123ms | 0 |
| `redis_lock` | 178 | 411ms | 0 |

여섯 가지 모두 중복 할당은 없었다. 속도보다 이걸 먼저 확인한다.

측정하면서 **후보를 분산했더니 데드락이 나는 것**을 발견했다.
재시도 루프가 트랜잭션 안에 있어 실패한 시도의 락이 쌓이고 있었다
(InnoDB 기록상 한 트랜잭션이 11~12개의 행 락 보유).
시도 하나를 트랜잭션 하나로 바꾸자 데드락이 사라지고 p99 도 3배 좋아졌다.

→ 전체 분석: [`bench/RESULTS.md`](bench/RESULTS.md)
