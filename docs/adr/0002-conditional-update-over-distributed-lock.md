...
이 결정이 맞는지 `bench/` 에서 재보았다 → [측정 결과](../../bench/RESULTS.md)

- 같은 문제에 Redis 락을 쓰면 처리량이 오히려 **1/4** 로 떨어졌다.
  경합은 그대로인데 네트워크 왕복만 두 번 늘기 때문이다.
- `SELECT ... FOR UPDATE` 는 p50 83ms / **p99 442ms**. 평균만 보면 안 보이는 대기가 쌓인다.
- 조건부 UPDATE 는 **재시도를 트랜잭션 안에서 돌리면 실패한 시도의 락이 쌓여
  데드락이 났다**(600회 중 28건). 시도 하나를 트랜잭션 하나로 바꾸자
  데드락이 사라지고 p99 도 399ms → 123ms 로 떨어졌다.

마지막 항목이 이 레포의 구현에도 그대로 적용된다.
`cash.hold()` 의 낙관적 재시도는 잔액 계정 행 하나만 건드리고
호출자가 트랜잭션 경계를 잡으므로 순환 대기가 생기지 않지만,
**재시도 범위를 넓히는 순간 같은 문제가 생긴다**는 걸 염두에 둬야 한다.
